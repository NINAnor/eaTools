...
```{r exampel-data-zoom, fig.cap="Example data produced by zooming in on four grid cells in the above figure."}
...
We now want to calculate the area weighted mean for each homogeneous area class, i.e. the three different colored grid cells in Fig. \@ref(fig:exampel-data-zoom).
...
We can also use the same function to return the values, including some summary statistics, as a dataframe:
```{r}
